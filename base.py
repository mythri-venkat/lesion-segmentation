...
def select_training_voxels(input_masks, threshold=2, datatype=np.float32):
    ...
    images = [nd.zoom(load_nii(image_name).get_data(),[0.25,0.25,0.25])[24:104,24:104,24:104] for image_name in input_masks]
...
def load_train_patches(x_data, y_data, selected_voxels, patch_size, random_state = 42, datatype=np.float32):
    ...
    images = [nd.zoom(load_nii(name).get_data(),[0.25,0.25,0.25])[24:104,24:104,24:104] for name in x_data]
    images_norm = [(im.astype(dtype=datatype) - im[np.nonzero(im)].mean()) / im[np.nonzero(im)].std() for im in images]

    # load labels 
    lesion_masks = [nd.zoom(load_nii(name).get_data(),[0.25,0.25,0.25])[24:104,24:104,24:104] for name in y_data]
...
def load_test_patches(test_x_data, patch_size, batch_size, voxel_candidates = None, datatype=np.float32):
    ...
    for m in modalities:
        raw_images = [nd.zoom(load_nii(test_x_data[s][m]).get_data(),[0.25,0.25,0.25])[24:104,24:104,24:104] for s in scans]
...
def test_scan(model, test_x_data, options, save_nifti= True, candidate_mask = None):
    ...
    seg_image = np.zeros_like(nd.zoom(flair_image.get_data(),[0.25,0.25,0.25])[24:104,24:104,24:104])
...
def select_voxels_from_previous_model(model, train_x_data, options):
    ...
    flair_scans = [train_x_data[s]['FLAIR'] for s in scans]
    images = [nd.zoom(load_nii(name).get_data(),[0.25,0.25,0.25])[24:104,24:104,24:104] for name in flair_scans]
